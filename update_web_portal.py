...
<head>
  ...
  <style>
    ...
    /* ID Pass Card Exact Replica Design */
    .id-pass-card {
      width: 100%;
      max-width: 650px;
...
<body class="bg-slate-100 text-slate-800 dark:bg-pup-darkBg dark:text-slate-100 min-h-screen transition-colors duration-200">
    ...
    <div class="max-w-md w-full bg-white dark:bg-pup-darkSurface rounded-3xl shadow-xl border border-slate-200 dark:border-red-950/50 p-6 sm:p-8 relative overflow-hidden">
      ...
      <form id="form-admin" onsubmit="handleAdminLogin(event)" class="space-y-4">
        <div>
          ...
          <div class="relative">
            ...
            <input type="text" id="admin-user" required placeholder="admin" value="admin" class="w-full pl-9 pr-4 py-2.5 rounded-xl border border-slate-300 dark:border-red-900/40 bg-white dark:bg-pup-darkCard text-sm focus:outline-none focus:ring-2 focus:ring-pup-maroon dark:focus:ring-pup-gold">
          </div>
        </div>
        <div>
          ...
          <div class="relative">
            ...
            <input type="password" id="admin-pass" required placeholder="••••••••" value="jmapup" class="w-full pl-9 pr-4 py-2.5 rounded-xl border border-slate-300 dark:border-red-900/40 bg-white dark:bg-pup-darkCard text-sm focus:outline-none focus:ring-2 focus:ring-pup-maroon dark:focus:ring-pup-gold">
          </div>
        </div>
      ...
      <form id="form-partner" onsubmit="handlePartnerLogin(event)" class="space-y-4 hidden">
        ...
        <div>
          ...
          <input type="password" id="partner-pin" maxlength="4" placeholder="••••" required class="w-full px-4 py-2.5 rounded-xl border border-slate-300 dark:border-red-900/40 bg-white dark:bg-pup-darkCard text-lg text-center font-mono tracking-[0.5em] focus:outline-none focus:ring-2 focus:ring-pup-gold">
        </div>
    ...
    <div class="bg-white dark:bg-pup-darkSurface max-w-3xl w-full rounded-3xl p-6 shadow-2xl border border-slate-200 dark:border-red-900 relative space-y-6 overflow-y-auto max-h-[90vh]">
      ...
      <div id="printable-id-card" class="id-pass-card">
        ...
        <div class="id-pass-header">
          ...
          <div class="text-[10px] font-extrabold tracking-widest text-white/90 uppercase">
        ...
        <div class="id-pass-body">
          <div class="id-pass-info">
            <div>
              ...
              <div id="pass-card-name" class="id-pass-val text-lg">Claudette T. Cansanay</div>
            </div>
            ...
            <div>
              ...
              <div id="pass-card-dept" class="id-pass-val text-sm">BSBA - Marketing Management</div>
            </div>

            <div>
              <div class="id-pass-label">VALID SEMESTER</div>
              <div id="pass-card-badge" class="id-pass-badge mt-1">
          ...
          <div class="id-pass-qr-frame">
            ...
            <span class="text-[9px] font-bold text-pup-maroon tracking-wider mt-1 uppercase">SCAN TO VALIDATE</span>
          </div>
        </div>
    ...
    <div class="bg-white dark:bg-pup-darkSurface max-w-md w-full rounded-3xl p-6 shadow-2xl border border-slate-200 dark:border-red-900 space-y-4">
      ...
      <form id="form-student" onsubmit="saveStudent(event)" class="space-y-3 text-xs font-semibold">
        ...
        <div>
          ...
          <input type="text" id="stu-name" required placeholder="Juan Dela Cruz" class="w-full p-2.5 rounded-xl border border-slate-300 dark:border-red-900/40 bg-slate-50 dark:bg-pup-darkCard text-sm focus:outline-none">
        </div>
        <div>
          <label class="block mb-1 text-slate-600 dark:text-slate-300">PUP Webmail</label>
          <input type="email" id="stu-email" placeholder="juan.delacruz@iskolarngbayan.pup.edu.ph" class="w-full p-2.5 rounded-xl border border-slate-300 dark:border-red-900/40 bg-slate-50 dark:bg-pup-darkCard text-sm focus:outline-none">
        </div>
        <div>
          <label class="block mb-1 text-slate-600 dark:text-slate-300">Department / Program</label>
          <input type="text" id="stu-dept" value="BSBA - Marketing Management" class="w-full p-2.5 rounded-xl border border-slate-300 dark:border-red-900/40 bg-slate-50 dark:bg-pup-darkCard text-sm focus:outline-none">
        </div>
        <div>
          <label class="block mb-1 text-slate-600 dark:text-slate-300">Active Semester</label>
          <input type="text" id="stu-sem" value="2026-2027 | 1st Semester" class="w-full p-2.5 rounded-xl border border-slate-300 dark:border-red-900/40 bg-slate-50 dark:bg-pup-darkCard text-sm focus:outline-none">
        </div>
    ...
    <div class="bg-white dark:bg-pup-darkSurface max-w-md w-full rounded-3xl p-6 shadow-2xl border border-slate-200 dark:border-red-900 space-y-4">
      ...
      <form id="form-store" onsubmit="saveStore(event)" class="space-y-3 text-xs font-semibold">
        ...
        <div>
          ...
          <input type="text" id="store-name" required placeholder="JMAP Food Hub" class="w-full p-2.5 rounded-xl border border-slate-300 dark:border-red-900/40 bg-slate-50 dark:bg-pup-darkCard text-sm focus:outline-none">
        </div>
        <div>
          <label class="block mb-1 text-slate-600 dark:text-slate-300">Category</label>
          <input type="text" id="store-cat" value="Food & Beverage" class="w-full p-2.5 rounded-xl border border-slate-300 dark:border-red-900/40 bg-slate-50 dark:bg-pup-darkCard text-sm focus:outline-none">
        </div>
        <div>
    ...
    <div class="bg-white dark:bg-pup-darkSurface max-w-md w-full rounded-3xl p-6 shadow-2xl border border-slate-200 dark:border-red-900 space-y-4">
      ...
      <form id="form-voucher" onsubmit="saveVoucher(event)" class="space-y-3 text-xs font-semibold">
        ...
        <div>
          ...
          <input type="text" id="vouch-title" required placeholder="10% Off All Orders" class="w-full p-2.5 rounded-xl border border-slate-300 dark:border-red-900/40 bg-slate-50 dark:bg-pup-darkCard text-sm focus:outline-none">
        </div>
        <div>
          <label class="block mb-1 text-slate-600 dark:text-slate-300">Description</label>
          <textarea id="vouch-desc" rows="2" class="w-full p-2.5 rounded-xl border border-slate-300 dark:border-red-900/40 bg-slate-50 dark:bg-pup-darkCard text-sm focus:outline-none">Valid for active JMAP members once per month.</textarea>
        </div>
  ...
  <script>
    ...
    // --- RENDER ROSTER (STUDENT DIRECTORY) ---
    ...
    // --- VIEW MEMBER PASS ID CARD (EXACT REPLICA) ---
    ...
    async function onScanSuccess(decodedText) {
      ...
      badge.className = "inline-block px-5 py-1.5 rounded-full text-xs font-black mb-3 bg-slate-200 text-slate-800";
      ...
      try {
        ...
        const docRef = await getDoc(doc(window.db, "students", studentId));

        let status = "NOT_FOUND";
        let studentData = null;
        let notes = window.appState.userRole === 'admin' ? "Scanned at Gate Entrance" : `Scanned at ${window.appState.partnerData.name}`;

        if (docRef.exists()) {
          studentData = docRef.data();
          status = studentData.isMembershipPaid ? "VALID" : "PAYMENT_PENDING";
        }

        nameEl.innerText = studentData ? studentData.fullName : "Unregistered Student";
        semEl.innerText = studentData ? studentData.activeSemester : "N/A";

        if (status === "VALID") {
          badge.className = "inline-block px-6 py-2 rounded-full text-sm font-extrabold mb-3 bg-emerald-100 text-emerald-800 dark:bg-emerald-950 dark:text-emerald-300 border-2 border-emerald-500 shadow-lg";
          badge.innerText = "VALID MEMBER";
          resDiv.className = "mt-6 p-6 rounded-2xl border-4 border-emerald-500 bg-emerald-50/50 dark:bg-emerald-950/20 text-center animate-fade-in";
        } else if (status === "PAYMENT_PENDING") {
          badge.className = "inline-block px-6 py-2 rounded-full text-sm font-extrabold mb-3 bg-amber-100 text-amber-800 dark:bg-amber-950 dark:text-amber-300 border-2 border-amber-500 shadow-lg";
          badge.innerText = "PENDING DUES";
          resDiv.className = "mt-6 p-6 rounded-2xl border-4 border-amber-500 bg-amber-50/50 dark:bg-amber-950/20 text-center animate-fade-in";
        } else {
          badge.className = "inline-block px-6 py-2 rounded-full text-sm font-extrabold mb-3 bg-red-100 text-red-800 dark:bg-red-950 dark:text-red-300 border-2 border-red-500 shadow-lg";
          badge.innerText = "INVALID / NOT FOUND";
          resDiv.className = "mt-6 p-6 rounded-2xl border-4 border-red-500 bg-red-50/50 dark:bg-red-950/20 text-center animate-fade-in";
        }
        ...
        console.error(err);
        badge.innerText = "ERROR VERIFYING";
      }
    }
    ...
    function renderScanLogs() {
      ...
      if (logs.length === 0) {
        container.innerHTML = `<tr><td colspan="5" class="py-8 text-center text-slate-400">No scan history recorded yet.</td></tr>`;
        return;
      }

      container.innerHTML = logs.map(l => {
        const timeStr = l.scannedAt ? new Date(l.scannedAt).toLocaleString() : 'N/A';
        let badge = `<span class="px-2 py-0.5 rounded text-[10px] font-bold bg-emerald-100 text-emerald-800">VALID</span>`;
        if (l.status === 'PAYMENT_PENDING') badge = `<span class="px-2 py-0.5 rounded text-[10px] font-bold bg-amber-100 text-amber-800">PENDING</span>`;
        if (l.status === 'NOT_FOUND') badge = `<span class="px-2 py-0.5 rounded text-[10px] font-bold bg-red-100 text-red-800">NOT FOUND</span>`;

        return `
          <tr class="hover:bg-slate-50 dark:hover:bg-pup-darkCard/50 transition">
            <td class="py-3 px-4 font-mono text-slate-400">${timeStr}</td>
            <td class="py-3 px-4 font-bold text-slate-900 dark:text-slate-100">${l.studentName}</td>
            <td class="py-3 px-4 font-mono text-pup-maroon dark:text-pup-gold">${l.studentId}</td>
            <td class="py-3 px-4">${badge}</td>
            <td class="py-3 px-4 text-slate-500">${l.notes || ''}</td>
          </tr>
        `;
      }).join('');
    }
    ...
    function renderStores() {
      ...
      if (stores.length === 0) {
        container.innerHTML = `<div class="col-span-full py-12 text-center text-slate-400">No partner stores added yet.</div>`;
        return;
      }

      container.innerHTML = stores.map(s => `
        <div class="bg-white dark:bg-pup-darkSurface p-5 rounded-2xl border border-slate-200 dark:border-red-950/60 shadow-sm flex justify-between items-center">
          <div>
            <h3 class="font-extrabold text-base text-pup-maroon dark:text-pup-gold">${s.name}</h3>
            <p class="text-xs text-slate-500 dark:text-slate-400">${s.category || 'Store'}</p>
            <p class="text-xs font-mono font-bold text-slate-700 dark:text-slate-300 mt-2 bg-slate-100 dark:bg-pup-darkCard px-2.5 py-1 rounded-lg border border-slate-200 dark:border-red-900/40 inline-block">
              PIN: ${s.pin}
            </p>
          </div>
          <button onclick="deleteStore('${s.docId}')" class="p-2.5 bg-red-50 dark:bg-red-950/60 text-red-600 dark:text-red-400 rounded-xl hover:bg-red-100 transition">
    ...
    function renderVouchers() {
      ...
      if (vouchers.length === 0) {
        container.innerHTML = `<div class="col-span-full py-12 text-center text-slate-400">No discount vouchers distributed yet.</div>`;
        return;
      }

      container.innerHTML = vouchers.map(v => `
        <div class="bg-white dark:bg-pup-darkSurface p-5 rounded-2xl border border-slate-200 dark:border-red-950/60 shadow-sm relative overflow-hidden flex justify-between items-start">
          <div class="absolute top-0 left-0 bottom-0 w-2 bg-pup-gold"></div>
          <div class="pl-3">
            <span class="text-[10px] font-extrabold uppercase text-pup-maroon dark:text-pup-gold">${v.partnerName}</span>
            <h3 class="font-extrabold text-base text-slate-900 dark:text-slate-100">${v.title}</h3>
            <p class="text-xs text-slate-500 dark:text-slate-400 mt-1">${v.description}</p>
          </div>
          <button onclick="deleteVoucher('${v.docId}')" class="p-2 bg-red-50 dark:bg-red-950/60 text-red-600 dark:text-red-400 rounded-xl hover:bg-red-100 transition">
            <i class="fa-solid fa-trash text-xs"></i>
          </button>
        </div>
      `).join('');
    ...
    function editStudent(docId) {
      const s = window.appState.students.find(x => x.docId === docId);
      if (!s) return;

      document.getElementById('stu-doc-id').value = s.docId;
      document.getElementById('stu-id').value = s.studentId || s.docId;
      document.getElementById('stu-name').value = s.fullName || '';
      document.getElementById('stu-email').value = s.email || '';
      document.getElementById('stu-dept').value = s.department || '';
      document.getElementById('stu-sem').value = s.activeSemester || '';
      document.getElementById('stu-paid').checked = !!s.isMembershipPaid;
    ...
    async function saveStudent(e) {
      e.preventDefault();
      const docId = document.getElementById('stu-doc-id').value || document.getElementById('stu-id').value.trim();
    ...
    async function deleteStudent(docId) {
      if (!confirm("Are you sure you want to delete this student from Firestore?")) return;
      try {
    ...
    async function saveVoucher(e) {
      ...
      if (sel.selectedIndex <= 0) return alert("Select a partner store");
    ...
    async function deleteVoucher(docId) {
      if (!confirm("Delete discount voucher?")) return;
      try {
        const { doc, deleteDoc } = await import("https://www.gstatic.com/firebasejs/10.12.2/firebase-firestore.js");
